...
구현 완료 후 확인:
- [x] `bun run dev` - 개발 서버 정상 실행
- [x] 홈 화면에서 10개 콘텐츠 카드 표시
- [x] 각 콘텐츠 클릭 시 iframe 뷰어에서 정상 실행
- [x] 모드 토글(보기/창조) 동작 확인
- [x] 창조 모드에서 관심사/과목/학년 입력 후 콘텐츠 생성
- [x] 생성된 콘텐츠가 홈 화면에 추가됨
- [x] 모바일 화면에서 반응형 레이아웃 확인
