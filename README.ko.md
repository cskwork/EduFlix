...
모든 콘텐츠는 독립 실행 가능한 바닐라 HTML/CSS/JS 묶음입니다. 새 Studio 미리보기는 불투명 출처의 iframe을 사용합니다. 기존 뷰어는 호환성을 위해 `allow-scripts`와 `allow-same-origin`을 유지하므로 호스트 앱과의 격리를 보장하지 않습니다. 격리를 강화하는 작업은 공개 운영 전에 완료해야 할 과제로 남아 있습니다.
...
- **iframe 실행** — Studio 미리보기는 불투명 출처를 사용하며 기존 뷰어는 호환성 권한을 유지
...
`/studio`에서 설명·활동·퀴즈의 정답과 해설을 직접 편집하고, 브라우저에 저장하거나 JSON·오프라인 HTML 파일로 배포할 수 있습니다. LLM 설정 없이도 사용할 수 있습니다. 저장 범위와 AI 제작 연결은 [수업 작업실 사용법](docs/lesson-studio.md)을 확인하세요.
